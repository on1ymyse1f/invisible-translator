...
- 主翻译窗采用内容优先的 Claude 类工作台布局：输入区是唯一视觉主角，目标语言与外观收进稳定的右上控制区，主操作始终是“翻译并写入”；没有为单任务工具硬加侧栏，也不复制 Claude 的商标、图标或具体界面资产。
- 外观支持“跟随系统 / Claude 暖纸 / Claude 墨夜 / 赛博霓虹”即时切换。暖纸以奶油纸张、陶土色强调和高可读深色正文为主；墨夜使用偏暖炭黑；赛博霓虹采用近黑底、青色主强调、品红辅强调和低密度网格，同时保留高不透明内容卡片，避免装饰影响阅读。
- 同一主题令牌覆盖主翻译窗、紧凑翻译窗、AI 兼容边缘栏和选区翻译浮层；主题切换只影响显示，不改变本地翻译、隐私名单、OCR 边界或写回授权。
